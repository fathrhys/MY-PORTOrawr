...
export default function ProjectsClient({ allProjects }: { allProjects: any[] }) {
    ...
    const projects = useMemo(() => {
        return activeCat === "ALL" ? allProjects : allProjects.filter((p) => p.category === activeCat);
    }, [allProjects, activeCat]);

    const pick = projects[0] || null;
    const pickCat = pick?.category ?? "OTHER";
    // If pickStyle is undefined, fallback to a safe object to prevent errors
    const pickStyle = CATEGORY_STYLES[pickCat as keyof typeof CATEGORY_STYLES] || { coverTint: "bg-slate-900/10", chip: "bg-white text-slate-900", dot: "bg-slate-400", cardHover: "hover:ring-slate-300", link: "text-slate-600" };
        ...
        <div className="mx-auto max-w-6xl px-6 pb-20 pt-10">
                                ...
                                <div className="mt-5 flex flex-wrap gap-2">
                                    ...
                                    {CATS.map((c) => (
                                        <FilterPill
                                            ...
                                            label={LABEL[c.key as keyof typeof LABEL] || c.key}
                                        />
                                    ))}
                                </div>
                            </div>
                                ...
                                <div className="relative h-40">
                                    ...
                                    <div className="absolute left-5 top-5 flex items-center gap-2">
                                        <span className={`rounded-full px-3 py-1 text-xs font-semibold ${pickStyle.chip}`}>
                                            {LABEL[pickCat as keyof typeof LABEL] || pickCat}
                                        </span>
            ...
            <section className="mt-14">
                <Reveal>
                    <SectionHead
                        kicker="Library"
                        title={activeCat === "ALL" ? "All Projects" : `Category: ${LABEL[activeCat as keyof typeof LABEL] || activeCat}`}
                        desc={
                            activeCat === "ALL"
                                ? "Semua project disusun dari yang terbaru."
                                : `Semua project di kategori ${LABEL[activeCat as keyof typeof LABEL] || activeCat}.`
                        }
                ...
                <div className="mt-7 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {projects.length === 0 ? (
                        ...
                        projects.map((p, idx) => {
                            const st = CATEGORY_STYLES[p.category as keyof typeof CATEGORY_STYLES] || pickStyle;
                            const isWide = idx === 0;
